...
//show real or filtered clicks
	...
	//lets build the new keyword list
			...
			', $db_table, true, true, false, " $click_filtered GROUP BY ip_id ORDER BY clicks DESC", $_POST['offset'], true, true);
...
echo "IP" . "\t" . "Clicks" . "\t" . "Click Throughs" . "\t" . "LP CTR" . "\t" . "Leads" . "\t" . "S/U"  . "\t" . "Payout"  . "\t" . "EPC"  . "\t" . "Avg CPC"  . "\t" . "Income"  . "\t" . "Cost"  . "\t" . "Net" . "\t" . "ROI"  . "\n";

if ($total_rows > 0) {

				while ($click_row = $info_result->fetch_assoc()) {
					if (!$click_row['ip_name']) { 
						$html['ip'] = '[no ip]';    
					} else {
						$html['ip'] = $click_row['ip_name'];
					} 
