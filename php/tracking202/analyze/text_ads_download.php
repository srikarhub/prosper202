...
//show real or filtered clicks
	...
	//lets build the new keyword list
			...
			', $db_table, true, true, false, " $click_filtered GROUP BY 2ta.text_ad_id ORDER BY clicks DESC", false, false, true);
...
if ($total_rows > 0) {
				...
				while ($click_row = $info_result->fetch_assoc()) {
					if (!$click_row['text_ad_name']) {
						$click_row['text_ad_name'] = '[no text ad]';
					} else {
						$click_row['text_ad_name'] = $click_row['text_ad_name'];
					}
