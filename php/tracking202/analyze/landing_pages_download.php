...
//show real or filtered clicks
	...
	//lets build the new keyword list
			...
			$query = query('
						SELECT 
							2c.landing_page_id AS lp_id, 
							2lp.landing_page_nickname AS landing_page_name, 
							COUNT(*) AS clicks, SUM(2cr.click_out) AS click_out, 
			...
			', $db_table, true, true, false, " $click_filtered GROUP BY lp_id ORDER BY clicks DESC", $_POST['offset'], true, true);
...
header("Content-Disposition: attachment; filename=T202_lp_".time().".xls");
header("Pragma: no-cache");
header("Expires: -1");
	
	
echo "LP" . "\t" . "Clicks" . "\t" . "Click Throughs" . "\t" . "LP CTR" . "\t" . "Leads" . "\t" . "S/U"  . "\t" . "Payout"  . "\t" . "EPC"  . "\t" . "Avg CPC"  . "\t" . "Income"  . "\t" . "Cost"  . "\t" . "Net" . "\t" . "ROI"  . "\n";

if ($total_rows > 0) {

				while ($click_row = $info_result->fetch_assoc()) {

						if (!$click_row['landing_page_name']) { 
							$html['landing_page_name'] = '[direct link]';    
						} else {
							$html['landing_page_name'] = htmlentities($click_row['landing_page_name'], ENT_QUOTES, 'UTF-8');
