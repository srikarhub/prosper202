	...
	if ($user_row['user_cpc_or_cpv'] == 'cpv')  $cpv = true;
		...
		//lets build the new keyword list
			...
			$query = query('
						...
						LEFT OUTER JOIN 202_site_domains AS 2sd ON (2sd.site_domain_id = 2suf.site_domain_id)
			', $db_table, true, true, false, " $click_filtered GROUP BY 2sd.site_domain_id ORDER BY clicks DESC", $_POST['offset'], true, true);
			...
			<div class="row" style="margin-top: 10px;">
				...
				<div class="col-xs-6 text-right" style="top: -10px;">
					...
					<a style="font-size:12px;" target="_new" href="/tracking202/analyze/referers_download.php">
			...
			<table class="table table-bordered table-hover" id="stats-table">
				...
				<tr style="background-color: #f2fbfa;">
					<th colspan="2" style="text-align:left">Referer</th>
					<th>Clicks</th>
			...
			if ($total_rows > 0) {
				...
				while ($click_row = $info_result->fetch_assoc()) {
					if (!$click_row['referer_name']) {
						$html['referer'] = '[no referer]';
					} else {
						$html['referer'] = htmlentities($click_row['referer_name'], ENT_QUOTES, 'UTF-8');
						$html['referer'] = '<a target="_new" title="'.$click_row['site_domain_host'].'" href="'.$html['referer'].'">'.$click_row['site_domain_host'].'</a>';
					}
					...
					//roi
						...
						<tr>
							<td colspan="2" style="text-align:left; padding-left:10px;"><?php echo $html['referer']; ?></td>
...
<div class="col-xs-12 text-center">
	<div class="pagination" id="table-pages">
	    <ul>
			<?if ($query['offset'] > 0) {
					printf(' <li class="previous"><a class="fui-arrow-left" onclick="loadContent(\'/tracking202/ajax/sort_referers.php\',\'%s\',\'%s\');"></a></li>', $i, $html['order']);
				}
				...
				if ($query['pages'] > 1) {
					for ($i=0; $i < $query['pages']-1; $i++) {
						if (($i >= $query['offset'] - 10) and ($i < $query['offset'] + 11)) {
							...
							printf(' <li %s><a onclick="loadContent(\'/tracking202/ajax/sort_referers.php\',\'%s\',\'%s\');">%s</a></li>', $class, $i, $html['order'], $i+1);
							unset($class);
						}
					}
				}
				...
				if ($query['offset'] > 0) {
					printf(' <li class="next"><a class="fui-arrow-right" onclick="loadContent(\'/tracking202/ajax/sort_referers.php\',\'%s\',\'%s\');"></a></li>', $query['offset'] - 1, $html['order']);
				}
			?>
		</ul>
	</div>
